...
#define NANO_OS_PORT_DISABLE_INTERRUPTS()   __asm("clrpsw i")

/** \brief Enable interrupts */
#define NANO_OS_PORT_ENABLE_INTERRUPTS()    __asm("setpsw i")
...
#define NANO_OS_PORT_RAISE_PRIVILEDGES()    if (!g_nano_os.current_task->port_data.is_priviledged) { NANO_OS_PORT_SwitchToPriviledgedMode(); }

/** \brief Switch to unpriviledged mode  */
#define NANO_OS_PORT_LOWER_PRIVILEDGES()    if (!g_nano_os.current_task->port_data.is_priviledged) { NANO_OS_PORT_SwitchToUnpriviledgedMode(); }
...
#define NANO_OS_PORT_TASK_CONTEXT_STACK_FRAME_SIZE(task)      17u
...
/** \brief Switch the CPU to unpriviledged mode */
