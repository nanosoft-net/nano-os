...
void UART_Init(void)
{
    ...
    /* Enable IOs for Rx and Tx lines (PA21 and PB4, Peripheral A)*/
    PIOA->PIO_WPMR = PIO_WPMR_WPKEY_PASSWD;
    PIOA->PIO_ODR = (1u << 21u);
    PIOA->PIO_ABCDSR[0u] &= ~(1u << 21u);
    PIOA->PIO_ABCDSR[1u] &= ~(1u << 21u);
    PIOA->PIO_WPMR = (PIO_WPMR_WPKEY_PASSWD | (1u << 0u));
    PIOB->PIO_WPMR = PIO_WPMR_WPKEY_PASSWD;
    PIOB->PIO_ODR = (1u << 4u);
    PIOB->PIO_ABCDSR[0u] &= ~(1u << 21u);
    PIOB->PIO_ABCDSR[1u] &= ~(1u << 21u);
    PIOB->PIO_WPMR = (PIO_WPMR_WPKEY_PASSWD | (1u << 0u));

    /* Turn on power and clock for the UART */
    PMC->PMC_WPMR = PMC_WPMR_WPKEY_PASSWD;
    PMC->PMC_PCER0 |= (1u << 14u);
    PMC->PMC_WPMR = (PMC_WPMR_WPKEY_PASSWD | (1u << 0u));

    /* Disable receive and transmit */
    USART1->US_CR = (1u << 7u) | (1u << 5u);

    /* Disable register write protection */
    USART1->US_WPMR = US_WPMR_WPKEY_PASSWD;

    /* Configure UART baudrate = UARTCLK/(8 * (2 - OVER) * (CD + FP/8)) = 150000000 / (8 * (2 - 0) * (81 + 3/8)) => 115207bps */
    USART1->US_BRGR = (81u << 0u) | (3u << 16u);
    
    /* Configure transfer : 8bits, 1stop bit, no parity, no flow control */
    USART1->US_MR = (4u << 9u) | (3u << 6u);

    /* Enable register write protection */
    USART1->US_WPMR = (US_WPMR_WPKEY_PASSWD | (1u << 0u));
...
void UART_Receive(uint8_t* data, uint32_t data_len)
{
    ...
    {
        /* Save interrupt mask */
        uint32_t int_mask = USART1->US_IMR;
...
void USART1_Handler(void)
{
    ...
    while ((USART1->US_CSR & (1u << 0u)) != 0) /* RXRDY bit */
    {
    ...
    /* Transmit data */
    if ((USART1->US_CSR & (1u << 9u)) != 0) /* TXEMPTY bit */
    {
