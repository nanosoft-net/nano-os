...
void UART_Init(void)
{
    ...
    /* Enable IOs for Rx and Tx lines (PC11 and PC10, AF7)*/
    GPIOC->AFR[1u] &= ~((0x0Fu << 8u) | (0x0Fu << 12u));
    GPIOC->AFR[1u] |= ((7u << 8u) | (7u << 12u));
    GPIOC->MODER &= ~((3u << 20u) | (3u << 22u));
    GPIOC->MODER |= ((2u << 20u) | (2u << 22u));
    ...
    /* Configure UART baudrate = UARTCLK/(16 * USARTDIV) = 37500000 / (16 * 20.34) => 115200bps
       DIV_Fraction = 16 * 0.34 = 5.44 => 0x05
       DIV_Mantissa = 20 => 0x14
       => UASRT_DIV = 20,3125 => baudrate = 115385bps
   */
    USART3->BRR = (0x14u << 4u) | (0x05 << 0u);
...
void USART3_IRQHandler()
{
    ...
    while ((USART3->SR & ( 1u << 5u)) != 0) /* RXNE bit */
    {
